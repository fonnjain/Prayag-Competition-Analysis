...
- `prayag-price-finder` (Price Finder) at `/price-finder/`
...
**Cross-app navigation rule:** links from one artifact to another MUST be plain `<a href="/product-db/">` / `<a href="/analysis/">` / `<a href="/price-finder/">` with EXACT absolute proxy paths — full-page navigation. Do NOT use `import.meta.env.BASE_URL` (that's per-app and would resolve wrong) and do NOT use the wouter router (client-side routing can't cross artifacts).
